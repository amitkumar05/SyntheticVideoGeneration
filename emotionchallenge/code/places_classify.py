...
#type = sys.argv[2]
input=[img for img in glob.glob(image_dir+'*.jpg')]
...
mean_data = np.asarray(mean_blob.data, dtype=np.float32).reshape(
    (mean_blob.channels, mean_blob.height, mean_blob.width))


#mea = caffe.io.blobproto_to_array(mean_blob.data)[0]
#mea=mea.mean(1).mean(1)
#print(mea)

mean_data = mean_data.mean(1).mean(1)
...
#VGG = caffe.Classifier(model_file, deploy_prototxt,
 #                      mean=mean_data,
  #                     channel_swap=(2,1,0),
   #                    raw_scale=255,
    #                   image_dims=(256, 256))
...
#for i in range(100):
	...
	li = []
	li.append(categories[np.argmax(l[0])])
	li.append(max(l[0]))
	print(li)
	d[input[i]]=li
#	if(i%1000 == 0):
...
#	prediction = VGG.predict([input_image],oversample=False)
#	print(prediction)

	#print('predicted category  =' + format(categories[prediction.argmax()])
