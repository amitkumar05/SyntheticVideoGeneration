...
def parallel_loop_speaking(img1):
...
def parallel_loop_nonspeaking(img1):
...
p = Pool(10)
# print(speaking_img_dict.keys()[0:3])
# exit(0)
d = p.map(parallel_loop_speaking, speaking_img_dict.keys()[0:10])
combined_d = {}
for i in range(len(d)):
	combined_d[d[i].keys()[0]] = d[i][d[i].keys()[0]]
	# print(combined_d.keys())

d = p.map(parallel_loop_nonspeaking, nonspeaking_img_dict.keys()[0:10])
for i in range(len(d)):
	combined_d[d[i].keys()[0]] = d[i][d[i].keys()[0]]

print(combined_d.keys())

np.save('tmp.npy',combined_d)
